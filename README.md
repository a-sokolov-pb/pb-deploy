...
```text
{
  ...
  "dependencies"?: {
    "repository"?: {
	  "postfix"?: string -- значение, которое нужно приклеить к ссылке на артефакт (например ?raw=true)
	  "credentials"?: boolean -- флаг (по умолчанию true), что в запросах на скачивание артефакта добавляем ссылки на $nexus.login и $nexus.pass
	},
